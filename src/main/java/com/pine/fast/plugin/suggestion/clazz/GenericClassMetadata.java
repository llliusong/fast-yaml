...
public class GenericClassMetadata extends ClassMetadata {
    ...
    private void init(@NotNull PsiClassType type) {
        if (PsiCustomUtil.isValidType(type)) {
            childLookup = PsiCustomUtil.getSanitisedPropertyToPsiMemberWrapper(PsiCustomUtil.toValidPsiClass(type));
            if (childLookup != null) {
                        ...
                        if (lastQuerySegment) {
                            return wrappers.stream().map(wrapper -> wrapper
                                    .buildSuggestionForKey(module, fileType,
                                            unmodifiableList(GenericUtil.newListWithMembers(matchesRootTillParentNode, wrapper)),
