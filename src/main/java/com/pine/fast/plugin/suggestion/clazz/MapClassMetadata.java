...
public class MapClassMetadata extends ClassMetadata {
            ...
            return doWithKeyDelegateOrReturnNull(proxy -> {
                ...
                if (!isEmpty(matches)) {
                    return matches.stream().map(helper -> helper.buildSuggestionForKey(module, fileType,
                            GenericUtil.newListWithMembers(matchesRootTillParentNode, new MapKeySuggestionNode(helper)),
    ...
    private void init(Module module, PsiClassType type) {
        if (PsiCustomUtil.isValidType(type)) {
            Map<PsiTypeParameter, PsiType> typeParameterToResolvedType = PsiCustomUtil.getTypeParameters(type);
            ...
            if (keyTypeParam.isPresent()) {
                ...
                if (this.keyType != null) {
                    keyMetadataProxy = ClassSuggestionNodeFactory.newMetadataProxy(module, this.keyType);
                }
            }
            ...
            if (valueTypeParam.isPresent()) {
                ...
                if (this.valueType != null) {
                    valueMetadataProxy = ClassSuggestionNodeFactory.newMetadataProxy(module, this.valueType);
                }
            }
        }
    }
