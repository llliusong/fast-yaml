...
public class ArrayMetadataProxy implements MetadataProxy {
    ...
    ArrayMetadataProxy(Module module, @NotNull PsiArrayType type) {
        this.type = type;
        delegate = ClassSuggestionNodeFactory.newMetadataProxy(module, type.getComponentType());
    }
        ...
        return doWithDelegateAndReturn(delegate -> {
            ...
            if (directChildKeyMatch != null) {
                ...
                matchesRootTillParentNode
                        .add(new com.pine.fast.plugin.suggestion.clazz.IterableKeySuggestionNode((SuggestionNode) directChildKeyMatch));
            ...
            if (!isEmpty(matches)) {
                return matches.stream().map(helper -> {
                    ...
                    List<SuggestionNode> rootTillMe = GenericUtil.newListWithMembers(matchesRootTillParentNode,
                            new com.pine.fast.plugin.suggestion.clazz.IterableKeySuggestionNode((SuggestionNode) helper));
    ...
    private void doWithDelegate(com.pine.fast.plugin.suggestion.clazz.MetadataProxyInvoker targetInvoker) {
        if (delegate != null) {
