...
import com.pine.fast.plugin.suggestion.clazz.GenericClassMemberWrapper;
...
public class PsiCustomUtil {
    ...
    private static final Key<CachedValue<Map<String, GenericClassMemberWrapper>>>
            YAML_PLUGIN_PROPERTY_TO_CLASS_MEMBER_WRAPPER =
            create("yaml_plugin_property_to_class_member_wrapper");
    private static final Key<CachedValue<PsiType>>
            YAML_PLUGIN_ERASE_FREE_TYPE_PARAMETER_TYPE =
            create("yaml_plugin_eraseFreeTypeParameterType");
    private static final Key<CachedValue<PsiType>> YAML_PLUGIN_FIRST_PARAMETER_TYPE =
            create("yaml_plugin_firstParameterType");
    private static final Key<CachedValue<PsiType>> YAML_PLUGIN_RETURN_TYPE =
            create("yaml_plugin_returnType");
    ...
    public static SuggestionNodeType getSuggestionNodeType(PsiType type) {
        if (type == null) {
            ...
            return nodeType != null ? nodeType : SuggestionNodeType.UNKNOWN_CLASS;
        } else if (type instanceof PsiClassType) {
            SuggestionNodeType nodeType = getSuggestionNodeTypeForPrimitive(type);
            if (nodeType != null) {
                return nodeType;
            } else if (type.getCanonicalText().equals(JAVA_LANG_STRING)) {
                return SuggestionNodeType.STRING;
            }
            ...
            if (classResolveResult.isValidResult()) {
                ...
                if (psiClass.isEnum()) {
                    ...
                    return SuggestionNodeType.DOUBLE;
        ...
        return SuggestionNodeType.UNKNOWN_CLASS;
    }
    ...
    private static SuggestionNodeType getSuggestionNodeTypeForPrimitive(PsiType type) {
        if (PsiType.BOOLEAN.equals(type) || PsiType.BOOLEAN
            ...
            return SuggestionNodeType.BOOLEAN;
                ...
                .equals(PsiPrimitiveType.getUnboxedType(type))) {
            return SuggestionNodeType.BYTE;
            ...
            return SuggestionNodeType.DOUBLE;
                ...
                .equals(PsiPrimitiveType.getUnboxedType(type))) {
            return SuggestionNodeType.CHAR;
        }
        return null;
    }
    ...
    /**
     * Checks if the element is valid. If not, throws {@link com.intellij.psi.PsiInvalidElementAccessException} with a
     * meaningful message that points to the reasons why the element is not valid and may contain the stack trace when
     * it was invalidated.
     */
    ...
    public static Map<String, GenericClassMemberWrapper> getSanitisedPropertyToPsiMemberWrapper(
        ...
        if (psiClass != null) {
            return getCachedValue(psiClass, YAML_PLUGIN_PROPERTY_TO_CLASS_MEMBER_WRAPPER,
            ...
            if (isSimplePropertyGetter(method)) {
                ...
                if (acceptableMember != null)
                    memberNameToMemberWrapper
                            .put(SuggestionNode.sanitise(propertyName), new GenericClassMemberWrapper(acceptableMember));
            }
        }
    ...
    private static PsiType getFieldType(final PsiField field) {
        return getCachedValue(field, YAML_PLUGIN_ERASE_FREE_TYPE_PARAMETER_TYPE, () -> {
    ...
    private static PsiType getSetterArgumentType(@NotNull PsiMethod method) {
        return getCachedValue(method, YAML_PLUGIN_FIRST_PARAMETER_TYPE, () -> {
    ...
    private static PsiType getGetterReturnType(@NotNull PsiMethod method) {
        return getCachedValue(method, YAML_PLUGIN_RETURN_TYPE, () -> {
